...
describe("application handover browser history", () => {
  it("uses fetch before navigating so a POST is not left in browser history", () => {
    ...
    const styles = readFileSync("public/application-handover.css", "utf8");

    expect(source).toContain("await fetch(handoverForm.action");
    expect(source).toContain("controller.abort(), 10000");
    ...
    expect(styles).toContain("animation: reveal-progress 160ms ease 700ms forwards");
    expect(styles).toContain('body[data-handover-state="failed"] .handover-actions');
  });
});
