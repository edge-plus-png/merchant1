...
The target Merchant Portal validates the signature, issuer, ticket type, audience, target business, exact target origin, issued time, expiry, and nonce. It atomically consumes the nonce before creating an opaque, merchant-local HQ-managed session. Failed tickets create no session. A merchant-user request cannot exchange a ticket, and a merchant user cannot issue one.
