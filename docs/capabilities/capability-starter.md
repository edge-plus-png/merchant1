...
- **`/api/portal-launch` internally, mounted inside the public zone** — the endpoint that receives a signed launch ticket, verifies it per [`signed-launch-ticket.md`](signed-launch-ticket.md), and on success hands control to the independently deployed application. For Move the browser-visible endpoint is `/apps/move/api/portal-launch`.
...
- **Portal-owned zone configuration** — a capability remains independently deployed, but its browser-visible routes live under a unique Merchant Portal path such as `/apps/move`. Merchant Portal securely proxies that namespace to the configured staging or production upstream. The capability must set a matching base path so pages, assets, actions, redirects, and its path-scoped cookie remain inside its zone.
- **Cookie isolation at the proxy boundary** — Merchant Portal forwards only the capability's own session cookie into its upstream zone. Portal and HQ cookies are removed before the external rewrite, and capability cookies use the application's unique path rather than `/`.
- **Authenticated public-origin assertion** — Vercel rewrites replace ordinary forwarded-host headers with the upstream host. Portal therefore supplies the browser-visible origin with a shared `MOVE_PROXY_SECRET`, and Move ignores that assertion unless the secret matches. The value must be identical in Merchant Portal and its matching Move deployment and must never be exposed to the browser.
...
- **Direct-origin protection** — the capability rejects requests to its authenticated routes without a valid launched session. Its upstream deployment remains an implementation endpoint, not a second user-facing login surface.
