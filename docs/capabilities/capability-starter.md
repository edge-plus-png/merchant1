...
- **Sovereign-origin configuration** — a capability retains its own dedicated origin, deployment, session and data (see [`../platform/deployment-and-vercel-model.md`](../platform/deployment-and-vercel-model.md)). The public Merchant Portal gateway may route its canonical `/apps/{slug}/...` namespace to that origin according to [`portal-application-routing.md`](portal-application-routing.md). The capability never receives or constructs that mount prefix; it uses mount-neutral navigation and its stable manifest-declared asset prefix on every origin.
