...
The architecture is frozen. This does not mean it can never change — it means no new ADR is written unless implementation proves an existing ADR wrong. The default assumption for any new question is that it's answered by the existing decisions in [`decisions/`](decisions/); "we thought of something else" is not, on its own, grounds for a new decision record. A new ADR is warranted only when building or migrating a real capability surfaces a concrete case the current contract cannot actually satisfy. Until that happens, the work is building against what's already decided, not continuing to design it.
...
0. [`platform/platform-overview.md`](platform/platform-overview.md) — one diagram, thirty seconds, read this first
